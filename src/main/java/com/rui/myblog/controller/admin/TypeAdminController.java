...
public class TypeAdminController {
    ...
    //  新增分类
    @PostMapping("/types")
    public String post(@Valid TypeDto type, RedirectAttributes attributes) {
        TypeDto type1 = typeService.getTypeByName(type.getName());
        if (type1 != null) {
    ...
    public String editPost(@Valid TypeDto type, RedirectAttributes attributes) {
        TypeDto type1 = typeService.getTypeByName(type.getName());
        if (type1 != null) {
            attributes.addFlashAttribute("message", "不能添加重复的分类");
            return "redirect:/admin/types/input";
